 ...
 * */
bool camera_initFromFile(std::string filename, ArduCamHandle &cameraHandle, ArduCamCfg &cameraCfg,int idxcam) {
	...
	int ret_val = ArduCam_open(cameraHandle, &cameraCfg,idxcam);
...
void readImage_thread(ArduCamHandle handle,int idxcam) {
	std::cout << "a" <<std::endl;
	ArduCamOutData* frameData;
	String  title2=String("ArduCam");
	String title=title2 + String(to_string(idxcam));
	std::cout << "c" <<std::endl;
	...
	while (_running) {
		...
		if (ArduCam_availableImage(handle) > 0) {
			int64 deltain_ms=getTimestamp()-beginTime;
			if(deltain_ms<0)deltain_ms+=1000;
				
			std::cout<<"time in single capture"<< "device id= "<<idxcam <<"fps :"<<(1.0/deltain_ms*1000)<<std::endl;
...
#endif
	//read config file and open the camera.
	if (camera_initFromFile(config_file_name, cameraHandle, cameraCfg,idxcam)) {
		ArduCam_setMode(cameraHandle, CONTINUOUS_MODE);
		Sleep(2000);
		camera_initFromFile(config_file_name, cameraHandle1, cameraCfg1,1);
		ArduCam_setMode(cameraHandle1, CONTINUOUS_MODE);
		Sleep(2000);
		//std::thread captureThread(captureImage_thread, cameraHandle);
		std::thread readThread(readImage_thread, cameraHandle, idxcam);
		//std::cout << "capture thread create successfully." << std::endl;
		std::cout << "read thread create successfully." << std::endl;
		std::thread readThread1(readImage_thread, cameraHandle1, 1);
		std::cout << "read thread 1 create successfully." << std::endl;
		int key = -1;
#ifdef linux
		while ((key = getchar()) != -1) {
#endif
